...
export function ProductCard({ product, onViewDetails }: ProductCardProps) {
  const { isLoggedIn, user, toggleFavorite, login } = useAuth();
  const { addToCart } = useCart();
  const [addedToCart, setAddedToCart] = useState(false);
  const [imageError, setImageError] = useState(false);

  const isAmazon = product.marketplace === "amazon";
  const isFavorite = isLoggedIn && user?.favorites?.includes(product.id);

  // Generate or slice history to last 7 days
  const recentHistory = useMemo(() => {
    let history: PriceHistoryEntry[] = product.priceHistory || [];
  ...
  // Determine trend
  const trendColor =
    recentHistory.length > 0 && recentHistory[0].price > product.price
      ? "#22c55e"
      : "#ef4444";
      ...
      >
        ...
        <div
          className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${
            isAmazon ? "bg-amazon/5" : "bg-ebay/5"
          }`}
        />
        ...
        {/* Marketplace Badge */}
        <div
          className={`absolute top-3 left-3 px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-wide uppercase flex items-center gap-1.5 ${
            isAmazon
              ? "bg-amazon/20 text-amazon border border-amazon/30"
              : "bg-ebay/20 text-ebay border border-ebay/30"
          }`}
        >
          <span className={`w-1.5 h-1.5 rounded-full ${isAmazon ? "bg-amazon" : "bg-ebay"}`} />
      ...
      <div className="p-5 flex flex-col flex-1">
          ...
          <div className="flex items-center gap-0.5">
            {[...Array(5)].map((_, i) => (
              <svg
                key={i}
                className={`w-4 h-4 ${
                  i < Math.round(product.rating) ? "text-yellow-400" : "text-dark-500"
                }`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
        ...
        <div className="mt-auto">
          <div className="flex justify-between items-end mb-3">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span
                  className={`text-xs font-medium flex items-center gap-1 ${
                    trendDown ? "text-green-400" : "text-red-400"
                  }`}
                >
                  <svg
                    className={`w-3 h-3 ${trendDown ? "" : "rotate-180"}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
            ...
            >
              <PriceHistoryChart data={recentHistory} width={70} height={30} color={trendColor} />
            </div>
          </div>
